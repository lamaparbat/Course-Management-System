...
public class Signup extends javax.swing.JFrame {
    ...
    private void initComponents() throws ClassNotFoundException, SQLException {
        ...
        //unvisible the courses dropdown
        courses.setVisible(false);

        //fill the courses name in dropdown
        fillCourseName();
    ...
    private void fillCourseName() throws ClassNotFoundException, SQLException {
        ArrayList<String> course_name_list = new Course_Backend().courseNameList();
        courses.addItem("Select a course");
        for (String i : course_name_list) {
            courses.addItem(i);
        }
    }
    ...
    private void createMouseClicked(java.awt.event.MouseEvent evt) throws ClassNotFoundException, SQLException {
        ...
        if (username_val.length() > 0 && email_val.length() > 0 && password_val.length() > 0 && phone_val.length() > 0 && mode_val.length() > 0 && mode_val != "Select User Mode") {
            ...
            boolean result = new User().createAccount(username_val, email_val, password_val, phone_val, mode_val, selected_course_name);
    ...
    private void modeItemStateChanged(java.awt.event.ItemEvent evt) {
        if (evt.getItem() == "Student") {
    ...
    private void coursesItemStateChanged(java.awt.event.ItemEvent evt) {
       selected_course_name = (String) evt.getItem();
    }
